<template>
    <div class="falco-date" :class="{ open: open }" @click="toggleOpen">
        <div class="falco-date-result" ref="resultElem">
            <input
                type="text"
                placeholder="Date"
                :value="modelValue"
        ...
        <transition name="falco-date-panel">
            <div
                v-show="open"
                class="falco-date-panel"
                @click.stop=""
                ref="panelElem"
            >
                <div class="falco-date-header">
                    <div>
                        <span class="falco-date-2prev" @click="stride(-1)"></span>
                        <span
                            v-show="level === PanelLevel.Day"
                            class="falco-date-prev"
                            @click="step(-1)"
                        ></span>
                    </div>
                    <div>
                        <span
                            class="falco-date-title"
                            :class="{
                                clickable: level !== PanelLevel.YearInterval,
                            }"
                        ...
                        <span
                            v-show="level === PanelLevel.Day"
                            class="falco-date-title clickable"
                    ...
                    <div>
                        <span
                            v-show="level === PanelLevel.Day"
                            class="falco-date-next"
                            @click="step(1)"
                        ></span>
                        <span class="falco-date-2next" @click="stride(1)"></span>
                    </div>
                </div>
                <!-- year interval -->
                <ul
                    v-show="level === PanelLevel.YearInterval"
                    class="falco-date-year-content"
                >
                    <li
                ...
                <div
                    v-show="level === PanelLevel.Day"
                    class="falco-date-day-content"
                >
                    <div class="falco-date-day-wrapper">
                        <ul class="falco-date-week">
                            <li>日</li>
                            <li>一</li>
                            <li>二</li>
                            <li>三</li>
                        ...
                        <ul class="falco-date-days">
                            <li
                                v-for="(item, i) in dayData"
                                :key="i"
                                class="falco-date-day-item"
                                :class="{
                                    preview: item.preview,
                                    'falco-date-today': item.today,
                                    selected: item.selected,
                                }"
                                @click="onClickDay(item.day)"
                            >
                                <span class="falco-date-day">{{ item.day }}</span>
                            </li>
                        </ul>
                    </div>
                    <div class="falco-date-today-btn" @click="onClickToday">
                        Today
                    </div>
                </div>
            </div>
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const toggleOpen = () => {
            ...
            if (modelValue.value) {
                [year.value, month.value, day.value] = parseDate(
                    modelValue.value
                );
            } else {
                const now = new Date();
            ...
            const { x, y, width, height } =
                resultElem.value!.getBoundingClientRect();
        ...
        const fillDayData = (yearOffset = 0, monthOffset = 0) => {
            ...
            for (let i = 0; i < firstDayWeek; i++) {
                const t = new Date(
                    firstDay.getTime() - (firstDayWeek - i) * 86400000
                );
                const ts = t.getTime();
                result.push({
                    day: t.getDate(),
