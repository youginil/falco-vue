  ...
  render() {
    ...
    if (this.$slots.default) {
      this.$slots
        .default()
        // @ts-ignore
        .filter((item) => item.type.name === 'BugTableColumn')
        .forEach((item) => {
          const propData = item.props;
          const propName = propData?.prop;
          const thChildren = [propData?.label || firstUpper(propName) || ''];
          let sortable = false;
            ...
            if (ob.prop === propName) {
              thChildren.push(
                h('span', {
                  class: [
                    'bug-table-order-by',
                    ob.order === 'asc' ? 'order-by-asc' : '',
                    ob.order === 'desc' ? 'order-by-desc' : '',
                  ],
                })
              );
              sortable = true;
          ...
          ths.push(
            h(
              'th',
              {
                class: ['bug-table-th', sortable ? 'sortable' : ''],
                style: {
                  // @ts-ignore
                  width: propData?.width ? propData.width : '',
                },
                ...thOn
              },
              thChildren
    ...
    const trs = this.list.map((item) => {
      ...
      return h('tr', { class: 'bug-table-row' }, tds);
    });
    const table = h('table', [h('thead', ths), h('tbody', trs)]);
    return h('div', { class: ['bug-table'] }, [table]);
  },
});
</script>
