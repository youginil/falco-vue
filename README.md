...
A **simple** **efficient** web components library base on Vue 3.

## Usage
```
import BugUI from 'bugui';
import 'bugui/style.css';

createApp(...).use(BugUI).mount('#app');
```
...
<template>
    <bug-dropdown>
        <template #default>Avatar</template>
        <template #dropdown>
            <bug-dropdown-item @click="goProfilePage">Profile</bug-dropdown-item>
            <bug-dropdown-item @click="exit">Exit</bug-dropdown-item>
        </template>
    </bug-dropdown>
</template>
<script lang="ts">
...
```
<bug-table :list="list" :sort="sort" @resort="onResort">
    <bug-table-column label="Username" prop="username">
        <template #default="item">{{ item.username }}</template>
    </bug-table-column>
    <bug-table-column label="Age" prop="age">
        <template #default="item">{{ item.age }}</template>
    </bug-table-column>
</bug-table>
